...
const createRefund = asyncHandler(async (req, res) => {
  try {
    const { originalTransaction, itemsReturned, refundAmount, status } =
      req.body;
    if (!originalTransaction || !itemsReturned || !refundAmount) {
      return res.status(400).json("fields are required");
    }
    const saleTransaction = await SalesTransaction.findById(
      originalTransaction
    );
    if (!saleTransaction) {
    ...
    res
      .status(500)
      .json({ error: "Failed to create refund", details: error.message });
  }
});
//get all refund
const getAllRefund = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get all refund", details: error.message });
  }
});
...
const getRefundById = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get refund", details: error.message });
  }
});
//update refund
const updateRefund = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update refund", details: error.message });
  }
});
//delete refund
const deleteRefund = asyncHandler(async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete refund", details: error.message });
  }
});
